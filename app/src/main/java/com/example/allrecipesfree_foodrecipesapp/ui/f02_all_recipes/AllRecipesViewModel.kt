...
class AllRecipesViewModel(
    ...
    val allPostsMenuOnlyData = MutableLiveData<List<RecipePosts>>()

    fun getAllPostsMenuOnlyData(isInternetConnected: Boolean) {
        showLoading.value = true
        viewModelScope.launch {
            when (val result =
                withContext(Dispatchers.IO) { getAllPostsMenuOnlyUseCase.execute(Unit, isInternetConnected) }) {
